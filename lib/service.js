...
let PDFFORMService = (function () {
    ...
    const _onPFBinDataReady = function (context, evtData) {
        ...
        const resData = new SvcResponse(200, "OK", evtData.pdfFilePath, "FormImage JSON");

        resData.Pages = recurseTextOnly(evtData);
        resData.Meta = evtData.Meta;

        // let pdfParser = this.get_parser();
        // resData.Fields = pdfParser.getAllFieldsTypes();
    ...
    const _onPFBinDataError = function (evtData) {
    ...
    cls.prototype.start = function () {
        ...
        const _postFilter = function (svcContext, filePath) {
            ...
            try {
                for (let key in request.files) {
                    if (request.files.hasOwnProperty(key)) {
                        ...
                        _postFilter(new SvcContext(request, response, next), tempFilePath);
